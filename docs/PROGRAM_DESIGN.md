...
벌림/오므림/이 보임/미소/F·V)을 직접 그렸다. `lib/speech.ts`의 `speak()`가
이미 노출하던 `onBoundary`(단어 경계) 이벤트가 올 때마다 이 6가지 중 직전과
다른 하나를 무작위로 골라 보여준다 — 실제 발음에 맞는 모양은 아니지만, 같은
모양만 반복하는 것보다 훨씬 "말하는 것처럼" 보인다. 상용 TTS로 교체되면
`onBoundary` 자리에 해당 API의 viseme 이벤트를 연결해 무작위 선택 대신 실제
음소에 맞는 모양을 고르도록 바꾸면 된다.
