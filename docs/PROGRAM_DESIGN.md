...
`components/Avatar2D.tsx`가 직접 그린 2D 일러스트 얼굴을 렌더링하고, TTS
발화 중 입모양을 바꾼다(처음엔 Three.js 3D 얼굴로 시도했다가, 평면 일러스트
스타일 요청에 맞춰 2D로 전환). §6.1이 전제한 "오디오 음소 타임스탬프 기반
viseme 시퀀스"는 두 가지 이유로 이번 프로토타입에서는 근사치로 대체했다:

1. `speechSynthesis`가 재생하는 오디오는 브라우저 표준상 Web Audio API의
   `AnalyserNode`에 연결할 방법이 없다(OS/브라우저 오디오 파이프라인 밖에서
   재생됨) — 그래서 실제 음성 파형을 읽어 입모양을 결정할 수 없다.
2. 음소별 정확한 viseme 타이밍을 받으려면 Azure Speech, Amazon Polly처럼
   viseme/스피치마크를 함께 반환하는 유료 TTS API가 필요하다(§7 로드맵의
   상용 TTS 도입 단계에 해당).

대신 애니메이션 업계의 "음소 그룹별 입모양" 관례(A/E/I, B/M/P, U, F/V 등을
몇 가지 대표 모양으로 묶는 방식)를 참고해 6가지 오리지널 입모양(닫힘/크게
벌림/오므림/이 보임/미소/F·V)을 직접 그렸다.

처음에는 `lib/speech.ts`의 `speak()`가 노출하던 `onBoundary`(단어 경계)
이벤트가 올 때마다 이 6가지 중 하나를 무작위로 고르는 방식으로 구현했다.
하지만 실제로 써보니 `onBoundary`는 브라우저·음성 조합에 따라(특히 일부
한국어 음성) **아예 발생하지 않는 경우가 있어**, "말하는 중" 상태인데도
입이 전혀 움직이지 않는 버그로 이어졌다. TTS 엔진이 이 이벤트를 언제·얼마나
자주 보낼지는 웹 표준이 보장하지 않으므로, 이벤트 발생 여부에 기대는 설계
자체가 신뢰할 수 없었던 것이다.
