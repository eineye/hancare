...
- **아바타**: Three.js로 렌더링한 3D 얼굴이 TTS 발화 리듬(단어 경계 이벤트)에 맞춰
  입을 움직입니다(`components/Avatar3D.tsx`) — 실제 음성 파형 분석이나 음소별
  viseme 매칭은 아닌 근사 립싱크입니다. 브라우저는 합성 음성 오디오를 Web Audio
  API로 분석할 방법을 제공하지 않아, 진짜 음량 기반 립싱크는 자체 TTS 서버(비seme
  타이밍 제공)가 있어야 가능합니다.
